...
class TeamTest extends TestCase
{
    ...
    /** @test */
    public function cannot_add_duplicate_member_to_team(): void
    {
        $team        = Team::factory()->create();
        $participant = Participant::factory()->create();

        // Se añade el participante por primera vez
        $this->actingAs($this->admin, 'sanctum')
            ->postJson("/api/teams/{$team->id}/members", [
                'participant_id' => $participant->id,
                'role'           => 'developer',
        ...
        // Se intenta añadir el mismo participante de nuevo; debe retornar 422
        $this->actingAs($this->admin, 'sanctum')
            ->postJson("/api/teams/{$team->id}/members", [
                'participant_id' => $participant->id,
                'role'           => 'developer',
