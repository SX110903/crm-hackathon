...
class EvaluationTest extends TestCase
{
    ...
    public function can_show_an_evaluation(): void
    {
        ...
        $this->actingAs($this->admin, 'sanctum')
            ...
            // Se verifica que el recurso incluye el promedio de las cuatro notas
            ->assertJsonStructure(['data' => ['total_score']]);
    }

    /** @test */
    public function create_fails_with_duplicate_project_judge_combination(): void
    {
        $project = Project::factory()->create();
        $judge   = Judge::factory()->create();
        ...
        // Segunda evaluación con el mismo par proyecto-juez: debe fallar con 422
        $this->actingAs($this->admin, 'sanctum')
            ->postJson('/api/evaluations', $payload)
            ->assertUnprocessable();
    }
